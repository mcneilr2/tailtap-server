export default [
 {
      id: 1,
      name: "The Plaza Skatepark (or) DownTown Skatepark",
      type: "Park",
      latitude: 49.277234,
      longitude:  -123.102575,
      address: "Quebec St & Union St, Vancouver, BC",
      hours: "24/7",
      is_public: true,
      is_lit_night: true,
      weather_coverage: "Partial",
      occupancy_level: 8,
      median_skill_level: 8,
      homepage_video_id: "gPpJ2lcCOxI",
      closest_hospital_name: "Mount Saint Joseph Hospital",
      closest_hospital_address: "3080 Prince Edward St, Vancouver, BC V5T 3N4",
    },
    {
      id: 2,
      name: "Leeside Skatepark",
      type: "Park",
      latitude: 49.28116307233384,
      longitude: -123.03134769117345,
      address: "Cassiar Connector & East Hastings St and Highway 1, Vancouver, BC V5K 2C8",
      hours: "24/7",
      is_public: true,
      is_lit_night: true,
      weather_coverage: "Full",
      occupancy_level: 4,
      median_skill_level: 8,
      homepage_video_id: "66kelc8CmCs",
      closest_hospital_name: "Burnaby Hospital",
      closest_hospital_address: "3935 Kincaid St, Burnaby, BC V5G 2X6",
  },
    {
  id: 3,
  name: "Britannia Courts",
  type: "Park",
  latitude: 49.276099546233056,  
  longitude: -123.07319686482965,
  address: "1490 Venables St, Vancouver, BC V5L 4X6",
  hours: "24/7",
  is_public: true,
  is_lit_night: true,
  weather_coverage: "None",
  occupancy_level: 9,
  median_skill_level: 5,
  homepage_video_id: "AfiScK4bZvg",
  closest_hospital_name: "Mount Saint Joseph Hospital",
  closest_hospital_address: "3080 Prince Edward St, Vancouver, BC V5T 3N4",
  },
];
